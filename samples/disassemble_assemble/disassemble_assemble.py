...
def main():
    ...
    b, size = Assemble__('mov dword ptr [eax+10], 0xdeadbeef')
    print repr(b)

    # Try to assemble random stuff
    try:
        b, size = Assemble__('where is my bawobab')
    except Exception, e:
        print e

    try:
        b, size = Assemble__('mov eax, kikoo')
    except Exception, e:
        print e

    b, size = Assemble__('mov dword ptr [esp+4], 0xBAB00 ; movups [eax], xmm0 ; inc ecx')
    print Disass(b)
